            ...
            <div className="space-y-3">
              {filteredAndSortedApplicants.map((app) => {
                ...
                return (
                  <div 
                    key={app._id}
                    className="rounded-xl border border-gray-400 bg-background-100 hover:border-gray-900 dark:hover:border-gray-100 p-4 sm:p-5 shadow-2xs transition-all flex flex-col gap-4"
                  >
                    ...
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {app.status === 'rejected' && (
                            <span className="px-2 py-0.5 rounded bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 font-bold text-[10px] uppercase tracking-wider shrink-0 flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-3 self-end md:self-auto shrink-0 border-t md:border-t-0 border-gray-300 dark:border-gray-800 pt-3 md:pt-0">
                      ...
                      <div className="text-right">
                        ...
                        {app.matchDetails?.pruningMultiplier !== undefined && app.matchDetails.pruningMultiplier < 1 && (
                          <div className="inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] font-mono font-bold border border-orange-500/30 text-orange-600 bg-orange-500/10 ml-2" title="Domain Pruning Penalty">
                            <span>x{app.matchDetails.pruningMultiplier.toFixed(2)}</span>
                          </div>
                        )}
                      </div>
                  ...
                  {/* AI Match Details (Reasoning & Pruning info) */}
                  {app.matchDetails?.reasoning && (
                    <div className="mt-2 pt-3 border-t border-gray-300 dark:border-gray-800">
                      <div className="flex items-center gap-1.5 mb-1.5">
                        <Sparkles className="w-3.5 h-3.5 text-purple-500" />
                        <span className="text-xs font-bold uppercase tracking-wider text-gray-900">AI Match Details</span>
                      </div>
                      <p className="text-[11px] font-mono text-gray-700 leading-relaxed">
                        {app.matchDetails.reasoning}
                        {app.matchDetails.pruningMultiplier !== undefined && app.matchDetails.pruningMultiplier < 1 && (
                          <span className="block mt-1 text-orange-600 dark:text-orange-400">
                            * General competency score was attenuated by a factor of {app.matchDetails.pruningMultiplier.toFixed(2)} due to strict Job Domain Specificity constraints and a domain relevance of {Math.round((app.matchDetails.candidateDomainRelevance || 0) * 100)}%.
                          </span>
                        )}
                      </p>
                    </div>
                  ...
                  {app.status === 'rejected' && app.requirementsReview?.length > 0 && (
                    <div className="mt-2 pt-3 border-t border-gray-300 dark:border-gray-800">
                      <div className="flex items-center gap-1.5 mb-2">
                        <AlertCircle className="w-3.5 h-3.5 text-red-500" />
                        <span className="text-xs font-bold uppercase tracking-wider text-gray-900">Requirement Failures</span>
                      </div>
                      <ul className="list-disc list-inside text-[11px] font-mono text-red-600 dark:text-red-400 space-y-1">
                        {app.requirementsReview.map((review, idx) => (
