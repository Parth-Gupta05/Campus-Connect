...
import { effect, frame, init, sampler, storage, surface, target, uniforms, texture } from 'vgpu';
...
export default function ShapeWaves({
  ...
  useEffect(() => {
    ...
    const reportFailure = error => {
      if (disposed || localFailed) return;
      localFailed = true;
    ...
    void (async () => {
      try {
        ...
        const sceneTarget = target(gpu, { size: initialSize, format: 'rgba8unorm', kind: '2d', label: 'shape-waves-scene' });
        const glowSize = size => [Math.max(1, Math.ceil(size[0] / 2)), Math.max(1, Math.ceil(size[1] / 2))];
        const glowA = target(gpu, { size: glowSize(initialSize), format: 'rgba8unorm', kind: '2d', label: 'shape-waves-glow-a' });
        const glowB = target(gpu, { size: glowSize(initialSize), format: 'rgba8unorm', kind: '2d', label: 'shape-waves-glow-b' });
        ...
        wakeRenderer = () => {
          if (disposed || localFailed || frameId) return;
        ...
        const drawMask = () => {
          ...
          if (hasText) {
            ...
            maskContext.fillText(content, maskWidth / 2 + textOffsetX, maskHeight / 2 + textOffsetY);
          }
        ...
        replayRef.current = () => {
          if (disposed || localFailed || !settingsRef.current.intro || reduceMotion.matches) return;
          introArmed = true;
          wakeRenderer();
        };
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        className={`pointer-events-none absolute inset-0 block h-full w-full opacity-100`}
      />
      {failed && (
        <div className="absolute inset-0 flex items-center justify-center text-xs text-red-500 font-mono bg-black/50 z-50 px-4 text-center">
          WebGPU Error: {typeof failed === 'string' ? failed : 'Failed to initialize'}
        </div>
      )}
    </div>
  );
