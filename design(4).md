...
## 0. Validation Against Official Documentation (vercel.com/geist)

> This section was added after cross-checking the file against Vercel's **official, current** Geist design-system docs (`vercel.com/geist/introduction`, `/geist/materials`, `/geist/typography`, `/geist/button`, and the live component nav). Everything below Section 1 was written from the marketing site, dashboard CSS observation, and third-party reconstructions — it is a reasonable visual approximation but is **not** the same artifact as the official Geist system, and several specific values conflict. Treat this section as authoritative where it overrides the body of the file.

### 0.1 Confirmed by official docs
- 4px spacing base, Geist Sans / Geist Mono font pairing, monochrome-first palette, restrained motion, single-blue-accent rule — all consistent with official framing.
- Component-driven system published as an installable package (`@vercel/geistcn`, icons via `@vercel/geistcn-assets`) rather than raw CSS you hand-roll.

### 0.2 Contradicted or superseded by official docs
- **Typography is not a raw px scale.** The official system (`/geist/typography`) is a named Tailwind class scale — `text-heading-*`, `text-button-*`, `text-label-*`, `text-copy-*` — with built-in Subtle/Strong modifiers via a nested `<strong>`. See §3.8, which replaces the px-based scale in §3.2–3.7 as the source of truth for anything built with the real Geist package.
- **Border radius is not "never 12px+ on interactive elements."** The official Materials spec (`/geist/materials`) explicitly assigns `material-medium` and `material-large` surfaces a **12px** radius, and floating elements (`material-menu`, `material-modal`) are **12px**, with `material-fullscreen` at **16px**. Only `material-base`/`material-small` and the tooltip use 6px. This directly contradicts the anti-pattern row in §34 ("Border radius 12px+ on interactive elements → Max 6–8px"). See §5.1.
- **The component library is much larger than what this file documents.** Sections 7–22 hand-build patterns (navbar, sidebar, buttons, forms, cards, tables, badges, tabs, dropdowns, modals, tooltips, popovers, toasts, empty states, code blocks, loading states) that already exist as official, named Geist components with their own APIs and written best-practice rules. The official component library (org by A–Z) is reproduced in full in the new §35, since it is the actual answer to "what UI templates does Geist provide."
- Several third-party "Vercel design system" reconstructions circulating online (used as inspiration for parts of this file) disagree with each other and with the official docs on core token values (e.g. quoting `#171717`/`#ffffff`/`#ebebeb` marketing-site tokens vs. this file's `#000000`/`#0A0A0A`/`#2E2E2E` dashboard tokens). Both can be independently true — one is the marketing site, the other is the authenticated dashboard — but they are not interchangeable, and neither is the same thing as the Geist *component* system in §35.
- **Iconography (§15) was unverifiable, not confirmed.** The official `/geist/icons` page is an interactive browser with no published size/stroke-width spec — the size tiers and 1.5px stroke in the original §15.1 are a plausible approximation, not a sourced fact. What *is* confirmed: the real import path `@vercel/geistcn-assets/icons`, the `Icon`+PascalCase naming convention, and a separate official **Geist Pixel** icon/type family this file previously omitted entirely. §15 now labels each claim by confidence level.

### 0.3 What this means practically
- For **marketing-site cloning** (hero sections, pricing pages, landing pages): the color/spacing/shadow observations in Sections 1–2, 4, 6, 25–28 remain a reasonable basis.
- For **building an actual product UI with Geist**: use the official component names and APIs in §35 and the official type scale in §3.8 rather than the hand-rolled CSS classes elsewhere in this file — they will not match what `@vercel/geistcn` actually renders.
...
### 3.8 Official Geist Typography Scale (validated against `vercel.com/geist/typography`)

This is the **actual, current** typeset system Geist ships as Tailwind utility classes — each class pre-sets `font-size`, `line-height`, `letter-spacing`, and `font-weight` together from the Geist Core Figma system. This is what to reach for in real `@vercel/geistcn` work; treat §3.2–3.7 as a visual approximation, not this system's source of truth.

**Usage pattern** — Subtle/Strong modifiers apply via a nested `<strong>`, not a separate class:
```html
...
- `text-heading-72`, `text-heading-64`, `text-heading-56`, `text-heading-48`, `text-heading-40`
- `text-heading-32` (Subtle), `text-heading-24` (Subtle), `text-heading-20` (Subtle), `text-heading-16` (Subtle)
- `text-heading-14`

**Buttons** — only within components that render buttons:
- `text-button-16` — largest button
- `text-button-14` — default button
- `text-button-12` — only for a tiny button placed inside an input field

**Label** — single-line text, ample line-height for pairing with icons:
- `text-label-20`, `text-label-18`
- `text-label-16` (Strong) — titles, to differentiate from regular text
- `text-label-14` (Strong) — the most common text style in the system; used in many menus
- `text-label-14-mono` — largest mono size, pairs with larger (>14) text
- `text-label-13` — secondary line next to other labels (tabular figures for numeric consistency)
- `text-label-13-mono` — pairs with Label 14 (smaller mono size looks better in that pairing)
- `text-label-12` — tertiary text in busy views: comments, Show More, calendar capitals
- `text-label-12-mono`

**Copy** — multi-line text, higher line-height than Label:
- `text-copy-24` (Strong), `text-copy-20` (Strong) — marketing hero areas
- `text-copy-18` (Strong) — marketing, large quotes
- `text-copy-16` (Strong) — simpler/larger views where text can breathe (e.g. Modals)
- `text-copy-14` (Strong) — the most commonly used text style overall
- `text-copy-13` — secondary text, space-constrained views
...
**Rules (this file's own convention):**
...
### 5.1 Official Geist Materials (validated against `vercel.com/geist/materials`)

Geist doesn't expose radius as a bare token — it bundles radius + fill + stroke + shadow into named **Material** presets. This is the authoritative version and it disagrees with the "never 12px+" rule above:

**Surface** — on the page:
| Material | Radius | Use |
|---|---|---|
| `material-base` | 6px | Everyday use |
| `material-small` | 6px | Slightly raised |
| `material-medium` | 12px | Further raised |
| `material-large` | 12px | Further raised |

**Floating** — above the page:
| Material | Radius | Use |
|---|---|---|
| `material-tooltip` | 6px | Lightest shadow; the only floating element with a triangular stem |
| `material-menu` | 12px | Lift from page |
| `material-modal` | 12px | Further lift |
| `material-fullscreen` | 16px | Biggest lift |

**Reconciliation:** treat `--radius-md: 6px` as correct for everyday dashboard surfaces (`material-base`/`material-small`), but drop the blanket "never 12px+" anti-pattern — `material-medium`, `material-large`, `material-menu`, and `material-modal` are all officially 12px, and fullscreen surfaces go to 16px.
...
> **⚠ Correction:** The size/stroke-width values below (14/16/20/24px, 1.5px stroke) are **not published by the official `/geist/icons` page** and could not be verified against it — that page is an interactive icon browser (search, "Copy Import," "Copy Name," "Copy JSX," "Copy SVG") with no prose spec sheet, a gap the Vercel community itself has flagged. Treat these numbers as a reasonable visual approximation only, not a sourced fact.

Custom icon set for developer tools, shipped as part of the official package (confirmed from `/geist/introduction`):

```tsx
import { IconPencilEdit, IconArrowUp } from '@vercel/geistcn-assets/icons';
```

- Naming convention: `Icon` + PascalCase name (e.g. `IconArrowUp`, `IconArrowLeft`, `IconArrowRight`, `IconPencilEdit`).
- **Unverified approximation** — Default size: 16px. Sizes: 14px (inline), 16px (default), 20px (medium), 24px (large).
- **Unverified approximation** — Stroke width: ~1.5px, outline/stroke style, rounded line caps, never filled/chunky.
- **Confirmed pattern (used elsewhere in official docs):** color follows `currentColor`, controlled by the parent's `color` — consistent with how icons are colored inside `Button` examples (`svgOnly` icon buttons inherit variant color).

### 15.2 Geist Pixel — a separate, official asset family (previously missing from this file)

Also published under `@vercel/geistcn-assets`, alongside the standard outline icons: **Geist Pixel**, a display typeface/icon family with five distinct pixel-grid variants, each exported separately (e.g. `font-pixel-circle`, `font-pixel-grid` utility classes observed in Vercel's own font CSS). Built for real product use (banners, dashboards, experimental layouts) rather than as a decorative gimmick — every glyph sits on a consistent pixel grid for legibility at small sizes. This is distinct from the outline icon set in §15.1 and should not be conflated with it.

### 15.3 Icon Usage Rules (unverified approximation — kept for visual consistency, not an official spec)
...
| Border radius 12px+ on *small* controls (inputs, buttons, badges) | 6px (`material-base`/`material-small`) is default; 12px is correct and official for `material-medium`/`material-large` surfaces, menus, and modals (§5.1) — it is not universally banned |
...
## 35. Complete Official Geist Component Library (A–Z)

Validated against the live `vercel.com/geist/<component>` docs and navigation (e.g. `/geist/button`, `/geist/toggle`, `/geist/pagination`, `/geist/show-more`, `/geist/theme-switcher`, `/geist/switch`). All components are imported from the barrel module:
...
Every docs page is also available as Markdown by appending `.md` to its URL (e.g. `/geist/button.md`), which is how this list was compiled and validated.

| Component | Purpose | Validated notes / best practice |
|---|---|---|
| **Avatar** | Represent a user or entity with an image or initials. | Pair with **Entity** when name + avatar + metadata appear together. |
| **Badge** | Small inline status or category label. | See also **Pill**, **Status Dot** for lighter-weight variants. |
| **Banner** / **Project Banner** | Full-width contextual message, often project- or account-scoped. | Distinct from **Toast** — Banner is persistent, Toast is transient. |
| **Book** | Documentation/book-style content container. | Used in docs-style surfaces. |
| **Breadcrumbs** | Hierarchical path navigation. | Pairs with **Pagination** for sequential nav — use Breadcrumbs for hierarchy, Pagination for prev/next. |
| **Browser** | Chrome-style browser-window mockup frame. | Marketing/demo use — wraps a screenshot or live preview. |
| **Button** | Trigger an action (deploy, save, delete) — see **ButtonLink** for navigation. | Default is primary. `type="secondary"` for supporting actions, `type="error"` for destructive confirmations. Use `loading` (not a swapped-in spinner) so it stays focusable. Icon-only buttons require both `svgOnly` and `aria-label`. Title Case labels naming the action (`Deploy Project`), not bare verbs (`Submit`). |
| **Calendar** | Date selection grid. | Pairs with **Select**/**Combobox** for date-range pickers. |
| **Checkbox** | Binary or indeterminate selection in forms/tables. | Use **Switch** instead when the action applies immediately (no Save step). |
| **Choicebox** | Card-style single/multi selection (richer than radio/checkbox). | Use when options need a description or icon, not just a label. |
| **Clearable Input** | Text input with a built-in clear affordance. | Superseded/merged in newer nav snapshots by **Input** variants — verify current API before using. |
| **Code** / **Code Block** | Inline or block syntax-highlighted code. | Code Block includes copy affordance, line numbers, language badge — the canonical dev-tool code presentation. |
| **Collapse** | Single expandable/collapsible section. | Use for optional sections; use **Show More** for progressive disclosure of one long list, **Pagination** for sibling pages. |
| **Combobox** | Searchable select with free-text filtering. | Use over **Select** when the option set is large or user-searchable. |
| **Command Menu** | Full-screen command palette, keyboard-first. | The reference dev-tool interaction — recent items, async search, keyboard nav. |
| **Context Card** | Rich hover/click-triggered preview card for an entity. | Lighter weight than a full **Modal**. |
| **Context Menu** | Right-click / long-press contextual action menu. | Distinct from **Menu** (persistent trigger) and **Dots Menu** (overflow trigger). |
| **Copy Button** / **Text With Copy Button** | One-click copy-to-clipboard affordance. | Pair with a brief toast confirmation. |
| **Description** | Label + value metadata row/list. | Common in detail panels and settings pages. |
| **Destructive Action Modal** | Confirmation modal specifically for irreversible actions. | Destructive CTA pairs 1:1 with its toast (`Delete Project` → `Project deleted`); keep destructive and constructive actions spatially separated. |
| **Dots Menu** | Overflow ("⋯") action trigger. | Use for row-level secondary actions in tables/lists. |
| **Drawer** | Side-anchored panel overlay. | Use over **Modal** for in-context editing that benefits from seeing the underlying page; use over **Sheet** on desktop, Sheet is the mobile-oriented equivalent. |
| **Empty State** | Placeholder shown when a list/section has no data yet. | Should include one clear primary action to populate the state. |
| **Entity** | Composite avatar + name + metadata row. | The standard "who/what" row across dashboards. |
| **Error** / **Error Card** | Inline or block error display. | Distinguish from **Feedback** (broader status messaging) and **Toast** (transient). |
| **Feedback** | General-purpose status/help messaging component. | Broader than Error — covers info/warning/success framing too. |
| **Fieldset** | Grouping wrapper for related form fields. | Pairs with **Label**, **Description**, **Input**. |
| **File Tree** | Hierarchical file/folder browser. | Dev-tool specific — repo browsers, deployment file viewers. |
| **Gauge** | Radial/arc progress or capacity indicator. | Use over **Progress** (linear) when framing usage against a limit. |
| **Grid** | Structural layout grid — "a huge part of the new Vercel aesthetic." | Use only when visible cell/guide layout is intentionally part of the design, not as a generic flex/grid replacement. |
| **Input** | Standard text field. | Sizes: 32/36/40px height tiers observed in this file's §4.3 remain a reasonable approximation. |
| **JSON View** | Collapsible, syntax-highlighted JSON viewer. | Dev-tool / API-response display. |
| **Keyboard Input** | Rendered keyboard-shortcut chip (e.g. `⌘K`). | Pairs with **Command Menu**. |
| **Label** | Form field label text. | Pairs with **Fieldset**, **Input**, **Description**. |
| **Load More Button** | Full-width button appending more items to a paginated list. | Has loading and styling variants; compare with **Show More** (in-place reveal) and **Pagination** (page-to-page nav). |
| **Loading Dots** | Inline animated ellipsis loading indicator. | Lighter-weight than **Spinner**; use inline in text/labels. |
| **Material** | The radius/fill/stroke/shadow preset system itself (§5.1). | `material-base/small/medium/large` (surface), `material-tooltip/menu/modal/fullscreen` (floating). |
| **Menu** | Persistent dropdown action/option list. | Distinct from **Context Menu** (right-click triggered) and **Command Menu** (full-screen palette). |
| **MiddleTruncate** | Text truncation that preserves the start and end (e.g. long hashes/paths). | Dev-tool specific — deployment URLs, commit SHAs. |
| **Modal** | Centered overlay dialog requiring user action/dismissal. | Use **Drawer**/**Sheet** instead for in-context or mobile-oriented flows. |
| **Multi Select** | Select supporting multiple chosen values. | Pairs with **Combobox** when the list is also searchable. |
| **Note** | Inline callout/tip block. | Non-blocking, informational — lighter than **Feedback**. |
| **Pagination** | Sequential navigation between sibling pages (docs, blog, onboarding steps). | Renders Previous/Next label, chevron, and `Go to {direction} page: {title}` aria-label automatically — don't prepend arrows or re-add "Go to." Hide (don't disable) the slot at sequence start/end. Use **Show More** instead for revealing more rows of the *same* data set. |
| **Phone** | Phone-frame mockup container. | Marketing/demo use, mobile-preview equivalent of **Browser**. |
| **Pill** | Rounded, lightweight status/category chip. | Lighter than **Badge**; often paired with **Status Dot**. |
| **Progress** | Linear progress indicator. | Use **Gauge** instead for arc/radial framing against a limit. |
| **Project Banner** | See **Banner** above. | |
| **Radio** | Single-choice selection among mutually exclusive options. | Use **Choicebox** when options need richer content than a label. |
| **Relative Time Card** | Hover-expandable relative timestamp (e.g. "2h ago" → absolute time). | Dev-tool pattern for deploy/commit timestamps. |
| **Scroller** | Custom-styled scrollable container. | Use for horizontally-scrolling row sets (e.g. tabs, chips) rather than default browser scrollbars. |
| **Search Input** | Input variant specialized for search (icon, clear, shortcut hint). | May appear merged into base **Input** in newer nav snapshots — verify current API. |
| **Select** | Dropdown list of items. | Use **Combobox** instead when searchable/filterable. |
| **Separator** | Visual divider line. | Structural only — no semantic meaning. |
| **Sheet** | Mobile-oriented anchored panel, conceptually paired with **Drawer**. | Prefer for mobile breakpoints; Drawer for desktop. |
| **Show More** | Progressive disclosure of a single long list/block. | Show 5–10 rows before truncating (2 feels performative). Pair the trigger with a count (`Show 12 More` → `Show Less`). Don't cycle Show More/Show Less mid-flow once expanded. Use **Pagination** instead for sibling pages of the same data set, **Collapse** for optional sections. |
| **Skeleton** | Placeholder loading shape mimicking eventual content layout. | Use over **Spinner**/**Loading Dots** when the final layout shape is already known. |
| **Slider** | Draggable range/value selector. | Pairs with a numeric **Input** for precise entry. |
| **Snippet** | Copyable single-line code/command block. | Lighter-weight than **Code Block** — typically one command (e.g. `npm install`). |
| **Spinner** | Indicate an action running in the background from a user action (buttons, pagination). | Distinct from **Loading Dots** (ambient/inline) — Spinner responds to a specific triggered action. |
| **Split Button** | Primary action button with an attached secondary dropdown of related actions. | Use when multiple related actions would otherwise crowd one row — see also **Button** best practices. |
| **Status Dot** | Small colored dot indicating state (online, error, deploying). | The only place small chromatic accent colors are used outside the primary blue. |
| **Switch** | Immediate on/off toggle. | Always provide a label via `children`, `aria-label`, or `aria-labelledby` — Geist warns in dev if all three are missing. Persist on change and confirm via toast rather than requiring a separate Save step. Label is Title Case, 1–4 words, naming what's true when ON (`Password Protection`, not `Enable Password Protection`). Pair icon-only Switches with a Tooltip. |
| **Table** | Structured tabular data display. | Pairs with **Pagination** or **Show More** for large datasets, **Dots Menu** for row actions. |
| **Tabs** | Switch between sibling views within the same context. | Use **Pagination** instead when moving between different pages/documents rather than views. |
| **Text With Copy Button** | See **Copy Button** above. | |
| **Textarea** | Multi-line text input. | Pairs with **Fieldset**, **Label**, **Description**. |
| **Theme Switcher** | Light/dark/system theme control. | Reads from and writes to `next-themes` — wrap the app in `GeistProvider` once at the root; don't mirror its state into local React state. Don't hand-roll a picker with **Switch** or icon buttons — it already handles per-option `aria-label`s and System detection. |
| **Toast** | Transient, auto-dismissing status notification. | Use for confirmations after an action (e.g. pairs with destructive-action toasts above). Distinct from persistent **Banner**. |
| **Toggle** | On/off control, similar to Switch but often used for view/filter states rather than settings. | Same labeling rules as Switch: Title Case noun phrase naming the ON state, disable only when the action is truly impossible (pair with a Tooltip explaining why). |
| **Tooltip** | Lightest-weight floating hint on hover/focus. | Uses `material-tooltip` (§5.1) — the only floating element with a triangular stem; official guidance elsewhere in this system notes a delay before appearing rather than instant display (§25). |

> **Note on nav drift:** Live crawls of `vercel.com/geist` taken at different times show slightly different component sets (e.g. some snapshots list **Clearable Input**, **Error Card**, **Fieldset**, **File Tree**, **JSON View**, **Label**, **Load More Button**, **Search Input**, **Separator**, **Text With Copy Button**, **Destructive Action Modal**, and **Dots Menu**; others don't). This table takes the **union** of all observed snapshots so nothing is missed — if a specific component 404s at `/geist/<name>`, it has likely been renamed or folded into a neighboring component (e.g. Copy Button ↔ Text With Copy Button); check the live nav at `/geist/introduction` before relying on any single one.

---

## 36. Design.md Table of Contents — As Structured on the Official Website

This is the actual content inventory of this file, cross-mapped to how `vercel.com/geist` organizes the same material. Sections 1–34 are this file's own marketing/dashboard reconstruction (kept for that purpose); §0, §3.8, §5.1, and §35 are the parts validated directly against the live official docs.

**Foundations** *(official: `/geist/introduction`, `/geist/colors`, `/geist/typography`, `/geist/materials`, `/geist/grid`)*
1. Design Philosophy
2. Color System
3. Typography *(§3.8 = official scale)*
4. Spacing System
5. Border Radius *(§5.1 = official Materials)*
6. Shadows & Depth

**Assets** *(official: `/geist/brands`, `/geist/icons`, `/font`)*
15. Icons

**Layout & Navigation Patterns** *(hand-built equivalents of official Grid/Menu/Command Menu/Breadcrumbs/Pagination)*
7. Navigation — Marketing Site Navbar
8. Navigation — Dashboard Sidebar
16. Tabs
28. Footer
...
**Components** *(official: `/geist/<component>` — full A–Z index now in §35)*
9. Buttons · 10. Forms & Inputs · 11. Cards & Panels · 12. Tables & Bulk Actions · 13. Badges & Status Tags · 14. Status Dots · 17. Dropdowns & Context Menus · 18. Modals & Dialogs · 19. Tooltips · 20. Popovers · 21. Toast Notifications · 22. Empty States · 23. Code Blocks & Snippets · 24. Loading States · **35. Complete Official Component Library (A–Z) — new**
...
31. Accessibility
32. Full CSS Custom Properties Reference
33. Reusability Guide — Shell Architecture
34. Anti-Patterns — What to Never Do

**Meta**
0. Validation Against Official Documentation — *new*
36. This table of contents — *new*

---

*Sources: `vercel.com` marketing site (scraped), `vercel.com/geist` design system, `vercel.com/geist/introduction`, `/geist/materials`, `/geist/typography`, `/geist/button`, `/geist/toggle`, `/geist/switch`, `/geist/pagination`, `/geist/show-more`, `/geist/theme-switcher`, `vercel.com/pricing`, `vercel.com/changelog` (Feb 2026 dashboard redesign), observed dashboard CSS variables, and third-party design system analysis cross-referenced against public Geist documentation. Last updated: May 2026; validated and extended September 12, 2026.*
